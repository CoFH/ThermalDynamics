...
import cofh.thermal.dynamics.attachment.FluidInputFilterAttachment;
...
import static cofh.thermal.dynamics.init.TDynContainers.FLUID_INPUT_FILTER_ATTACHMENT_CONTAINER;

public class FluidInputFilterAttachmentContainer extends AttachmentContainer implements IFilterOptions {

    public final FluidInputFilterAttachment attachment;

    protected BaseFluidFilter filter;
    protected InvWrapperFluids filterInventory;

    public FluidInputFilterAttachmentContainer(int id, Level world, BlockPos pos, Direction side, Inventory inventory, Player player) {

        super(FLUID_INPUT_FILTER_ATTACHMENT_CONTAINER.get(), id, world, pos, side, inventory, player);

        if (hostTile instanceof IDuct<?, ?> duct && duct.getAttachment(side) instanceof FluidInputFilterAttachment expectedAttachment) {
